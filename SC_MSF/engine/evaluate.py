...
def evaluate_multimodal(pred_traj, target_traj, mode='bbox', distribution=None, bbox_type='x1y1x2y2'):
    ...
    pred_traj: (B, T, K, D) or (B, K, T, D)
    target_traj: (B, T, D)
    '''
    if pred_traj.ndim != 4 or target_traj.ndim != 3:
        raise ValueError(f"Unexpected shapes: pred_traj {pred_traj.shape}, target_traj {target_traj.shape}")

    # Normalize predicted trajectory shape to (B, T, K, D)
    # Some codepaths produce (B, K, T, D).
    if pred_traj.shape[1] == target_traj.shape[1]:
        # (B, T, K, D)
        pass
    elif pred_traj.shape[2] == target_traj.shape[1]:
        # (B, K, T, D) -> (B, T, K, D)
        pred_traj = np.transpose(pred_traj, (0, 2, 1, 3))
    else:
        raise ValueError(f"Pred/target time dimension mismatch: pred_traj {pred_traj.shape}, target_traj {target_traj.shape}")
