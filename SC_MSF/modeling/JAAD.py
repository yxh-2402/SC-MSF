...
class ChannelAttention1D(nn.Module):
    ...
    def __init__(self, in_planes, ratio=16):
        ...
        self.fc1 = nn.Conv1d(in_planes, max(in_planes // ratio, 1), 1, bias=False)
        self.relu1 = nn.ReLU()
        self.fc2 = nn.Conv1d(max(in_planes // ratio, 1), in_planes, 1, bias=False)
...
class DTDEConv1D(nn.Module):
    ...
    def __init__(self, in_channels, kernel_size=3, use_wavelet=True):
        ...
        # 初始化低频卷积为均值滤波器
        if use_wavelet:
            nn.init.constant_(self.low_freq[0].weight, 1.0 / 3.0)
    ...
    def forward(self, x):
        ...
        # 2. 小波式分解：分离趋势和细节
        if self.use_wavelet:
            x_low = self.low_freq(x)           # 低频：运动趋势
            x_high = x - x_low                  # 高频：运动细节

            # 自适应高频增强
            high_weight = self.high_enhance(x_high)
            x_high = x_high * high_weight
        ...
        # 5. 残差连接
        out = self.fusion(x_attn) + identity
...
class JAAD(nn.Module):
    def __init__(self, cfg, dataset_name=None):
        super(JAAD, self).__init__()
        ...
        # 修改：h_m 现在是 256D（场景上下文记忆融合后）
        self.p_z_x = nn.Sequential(nn.Linear(self.hidden_size,  # 256D
                                            128),
                                    nn.ReLU(),
                                    nn.Linear(128, 64),
                                    nn.ReLU(),
        ...
        # posterior
        self.q_z_xy = nn.Sequential(nn.Linear(self.hidden_size + self.cfg.GOAL_HIDDEN_SIZE,  # 256 + 64 = 320D
                                            128),
                                    nn.ReLU(),
                                    nn.Linear(128, 64),
                                    nn.ReLU(),
...
##########3##########################################################################################################################
        ...
        self.traj_decoder = nn.Sequential(nn.Linear(self.hidden_size,
                                                    128),
                                          nn.ReLU(),
                                          nn.Linear(128, 64),
                                          nn.ReLU(),
        ...
        # ==================== 小波特征增强模块 (DTDEConv1D) ====================
        # 用于增强光流时序特征，捕捉运动趋势和细节
        self.flow_dtde = DTDEConv1D(in_channels=24, kernel_size=3, use_wavelet=True)
        # ==================== 小波特征增强模块结束 ====================
        ...
        #######################################################
        # ==================== 场景上下文记忆模块 (Scene Context Memory) ====================
        # 创新点：让场景特征引导多模态特征融合，实现场景感知的动态调制

        # 1. 场景特征增强编码器：将32D图像特征提升到256D
        ...
        # 3. FiLM条件调制：根据场景动态调整轨迹特征
        self.scene_gamma = nn.Sequential(
            nn.Linear(256, 256),
            nn.Sigmoid()  # 缩放因子 [0, 1]
        )
        self.scene_beta = nn.Linear(256, 256)  # 偏移因子
        ...
        self.tcn_p = TemporalConvNet(4, num_channels, kernel_size=8, dropout=0.1)  # K=2: 2关键帧+起始帧+当前帧=4帧
        self.tcn_f = TemporalConvNet(14, num_channels, kernel_size=8, dropout=0.1)
...
#########################################################################################################################
    ...
    def gaussian_latent_net(self, enc_h,h_x, cur_state, target=None, z_mode=None):
        ...
        # print(torch.sum(hx_conf[:,0])/hxe.shape[0] * 20)
        ...
        if tail_num_real != 0:
            if tail_num_real % 2 != 0:
                if tail_num_real == 1:
                    ...
                    K_samples_tail = -4 * K_samples_one + K_samples_t
                else:
        ...
        # Debug printing/plotting of latent samples (very expensive and may block training with GUI windows).
        debug_plot_z = os.environ.get("SC_MSF_DEBUG_PLOT_Z", "0") == "1"
        if debug_plot_z:
            print(K_samples[0])
            K_samples1 = torch.randn(enc_h.shape[0], self.K, self.cfg.LATENT_DIM, device=enc_h.device)

            data = K_samples.reshape(1, enc_h.shape[0] * 20 * 32).detach().cpu()
            data1 = K_samples1.reshape(1, enc_h.shape[0] * 20 * 32).detach().cpu()  # 数组数据
            # sns.histplot(data)
            # plt.hist(data1, 100)
    ...
    def encode_variable_length_seqs(self, original_seqs, key_pose,flow_seq_enc,lower_indices=None, upper_indices=None, total_length=None):
        '''
...
####################################################################################################################
        ...
        key_pose = key_pose.reshape(batch_size, 8,36).to(device) # 关键帧
        # ========== K=2: 起始帧 + 2个高置信度关键帧 + 当前帧 ==========
        # 论文配置：从15帧中保留起始帧和当前帧，选取置信度最高的2帧关键帧
        key_pose_seq = torch.zeros((batch_size, 4, 36)).to(device)
        key_pose_seq[:, 0, :] = str_pose           # 起始帧（固定）
        key_pose_seq[:, 1, :] = key_pose[:, 0, :]  # 置信度最高的关键帧1
        key_pose_seq[:, 2, :] = key_pose[:, 1, :]  # 置信度第二的关键帧2
        key_pose_seq[:, 3, :] = cur_pose           # 当前帧（固定）
        # ============================================================
        ...
        # print(key_pose_seq.shape)
        # print(key_pose_seq.shape) # 双向gru 输出维度（128，3，64））
        # key_pose_seq = key_pose_seq.reshape(batch_size, 128)
        # key_pose_seq = key_pose_seq[:,-1,:]
        # print(key_pose_hx.shape)
        # print(key_pose_seq.shape)
        ...
        flow_seq = flow[:,1:,:,:,:,:].reshape(batch_size,14,2,24,8).to(device)
        # f = self.f(flow_seq.reshape(batch_size, 5376)) ##为权重作准备
        f = self.tcn_f(flow_seq.reshape(batch_size, 14,384))[:,0,:]  ##为权重作准备
        f = self.f(f)
        ...
        # ==================== DTDEConv1D 增强光流时序特征 ====================
        # 转置为 (batch, channels, time) 格式
        flow_seq_dtde = flow_seq_zero.permute(0, 2, 1)  # (batch, 24, 14)
        # 小波特征增强：提取运动趋势和细节
        flow_seq_dtde = self.flow_dtde(flow_seq_dtde)
        # 转置回 (batch, time, channels) 格式
        flow_seq_zero = flow_seq_dtde.permute(0, 2, 1)  # (batch, 14, 24)
        # ==================== DTDEConv1D 增强结束 ====================
        ...
        img_h = img_h - img_h * obd_speed_e[0]
        # print(img_h.shape)
        ##################3#################################### 获取评价网络的权重

        # t = self.t(input_x.reshape(batch_size,60))
        # t = self.tcn_x(input_x)[:,0,:]
        # t = self.t(t)
        # print('t',t.shape,p.shape,f.shape)
        # p = self.p(p.reshape(batch_size, 144))
        # f=self.f(f.reshape(batch_size,5376))
        # w_seq = torch.cat((t, p, f), dim=1).to(device)
        # # w_seq = self.tpf(w_seq)
        # w_seq = w_seq.cpu().detach().numpy()
        ...
        #############################################################
        # x = torch.zeros((128,15,32)).to(device)
        # t = self.tcn()
        # # print('tcn_test',y.shape)


        #######################################################################
        # h_q = h_x
        h_y = h_x
        # h_y= torch.cat((h_y, flow_seq_enc), dim=1)  # (1,128,292)
        # h_y = torch.cat((h_y,key_pose_seq_cave), dim=1)  # (1,128,292)
        # # # h_x = self.attn(h_x,h_x,h_x).reshape(batch_size,324)
        # h_y = self.pf_cave(h_y)
        flow_seq_enc_y = flow_seq_enc
        key_pose_y = key_pose

        #################################################################
        key_pose = key_pose_seq_cave
        # key_pose = key_pose.cpu().detach().numpy()  #### 循环部分放到内存
        # flow_seq_enc = flow_seq_enc.cpu().detach().numpy()
        #
        #
        # for i in range(batch_size):
        #     h_x[i] = h_x[i] * w_seq[i][0]
        #     key_pose[i] = key_pose[i] * w_seq[i][1]
        #     flow_seq_enc[i] = flow_seq_enc[i] * w_seq[i][2]
        #     print(h_x.shape,key_pose.shape,flow_seq_enc.shape)
        # key_pose = torch.FloatTensor(key_pose).to(device)
        # flow_seq_enc = torch.FloatTensor(flow_seq_enc).to(device)
        # key_pose = key_pose * p
        # flow_seq_enc = flow_seq_enc * f
        # print(h_x.shape)
        # print(t.shape)
        # h_x = h_x * t[:,0,:]
        # print(h_x.shape)
        ...
        #### 循环部分从内存放出来
        #
        # h_q = torch.cat((h_q, flow_seq_enc), dim=1)  # (1,128,292)
        # h_q = torch.cat((h_q, key_pose), dim=1)  # (1,128,292)
        # h_x = self.attn(h_x,h_x,h_x).reshape(batch_size,324)
        # h_q = self.pf_cave(h_q)
        # h_x = self.attn_h(h_q,h_y,h_y).reshape(batch_size,256)
        # h_x = self.attn_h(h_y,h_q,h_q).reshape(batch_size,256) 效果不好
        ...
        # key_pose_y = self.pose_att(key_pose_y)
        # h_p = self.cattn(h_y, key_pose, key_pose)
        # h_x = self.cattn(h_p, flow_seq_enc, flow_seq_enc)
        h_p = self.cattn( key_pose_y* p, flow_seq_enc* f, flow_seq_enc_y* f)
        h_x = self.cattn(h_x  * t[:,0,:], h_p,h_y).reshape(batch_size,256)

        # ==================== 场景上下文记忆融合 ====================
        # 1. 增强场景特征：32D → 256D
        scene_feat = self.scene_encoder(img_h)  # (batch, 256)

        # 2. 构建多模态序列：[轨迹, 姿态, 光流]
        # 注意：这里的 key_pose 和 flow_seq_enc 已经是256D了
        multimodal_seq = torch.stack([h_x, key_pose, flow_seq_enc], dim=1)  # (batch, 3, 256)

        # 3. 场景引导注意力：场景Query查询多模态特征
        scene_query = scene_feat.unsqueeze(1)  # (batch, 1, 256)
        scene_context, scene_attn_weights = self.scene_guided_attn(
            scene_query,      # Query: 场景特征
            multimodal_seq,   # Key: 多模态特征
            multimodal_seq    # Value: 多模态特征
        )
        scene_context = scene_context.squeeze(1)  # (batch, 256)

        # 4. FiLM条件调制：根据场景动态调整轨迹特征
        gamma = self.scene_gamma(scene_feat)  # 缩放因子
        beta = self.scene_beta(scene_feat)    # 偏移因子
        h_x_modulated = gamma * h_x + beta    # 场景条件调制

        # 5. 融合场景上下文和调制后的轨迹特征
        h_m = self.scene_fusion(h_x_modulated + 0.1* scene_context)  # (batch, 256)
        # ==================== 场景上下文记忆融合结束 ====================
        ...
        # print(enc_h_and_z.shape)
        # enc_h_and_z.shape = [128,20,288]
...
################################################################################################################
    ...
    #     forward_h = self.enc_h_to_forward_h(dec_h)
    #     #################################################3
    #     # forward_h =  forward_h.reshape(bs,1,256)
    #     # for i in range(20):
    #     #     for t in range(pred_len):
    #     #         final_emb = self.pos_emb(forward_h.permute(1, 0, 2)).permute(1, 0, 2)
    #     #         forward_outputs  = self.transformer_encoder(final_emb)
    #     #         forward_h = torch.cat([forward_h,forward_outputs],dim=1)
    #     #         print(forward_outputs.shape)
    #
    ...
    #     # forward_outputs1 = torch.zeros((bs,45,20,256)).to(device)
    #     forward_outputs1 = torch.zeros((1, 45, 256)).to(device)
    #     for i in range(20):
    #         forward_outputs = []
    #         forward_h = forward_h.view(-1, forward_h.shape[-1])
    #         # print(forward_h.shape) #torch.Size([2560, 256]) 复制了20条轨迹
    #         forward_input = self.traj_dec_input_forward(forward_h)
    #         # print(forward_input.shape) ###
    #         for t in range(pred_len):  # the last step is the goal, no need to predict
    #             forward_h = self.traj_dec_forward(forward_input, forward_h)
    #             # print(forward_h.shape) # torch.Size([2560, 256])
    #             forward_input = self.traj_dec_input_forward(forward_h)
    #             # print(forward_input.shape)
    #             forward_outputs.append(forward_h)
    #         # torch.Size([2560, 45, 256])
    #         # print(forward_h.shape) #torch.Size([2560,  256])
    #         # final_emb = self.pos_emb(forward_outputs.permute(1, 0, 2)).permute(1, 0, 2)
    #         # forward_outputs = self.transformer_encoder(final_emb)
    #         # print( forward_outputs.shape ) #torch.Size([2560, 45, 256])
    #         forward_outputs = torch.stack(forward_outputs, dim=1)
    #         # print( forward_outputs.shape ) #torch.Size([2560, 45, 256])
    #         final_emb = self.pos_emb(forward_outputs)  # .permute(1, 0, 2)
    #         forward_outputs = self.transformer_encoder(final_emb)
    #         # print(forward_outputs.shape)  # torch.Size([2560, 45, 256])
    #         forward_outputs1 = torch.cat([forward_outputs, forward_outputs1], dim=0)
    #
    #     # print(forward_outputs1.shape)  # torch.Size([2560, 45, 256])
    #     forward_outputs = forward_outputs1[1:, :, :]
    #     ###############################################################
    #
    #     # print(Z.shape) #torch.Size([128, 20, 32])
    #     # print(enc_h_and_z.shape) #torch.Size([128, 20, 288])
    #
    #     # context =dec_h
    #     #
    #     # bs = x.size(0)
    #     # context = context.view(bs, 1, -1)  # (B, 1, F)
    #     # x = self.concat1(context, x)
    #     # final_emb = forward_outputs.permute(1, 0, 2)
    #     # final_emb = self.pos_emb(final_emb).permute(1, 0, 2)
    #     # print(final_emb.shape)
    #     # trans = self.transformer_encoder( final_emb).permute(1, 0, 2)
    #     # trans = self.concat3(context, trans)
    #     # trans = self.concat4(context, trans)
    #     # tran = self.linear(context, trans)
    #     # forward_outputs  = trans
    #     # print('f',forward_outputs[:, 1].shape) #[2560, 256]
    #     # 2. run backward on all samples
    #     # print(dec_h.shape)
    #     backward_outputs = []
    #     backward_h = self.enc_h_to_back_h(dec_h)
    ...
    #     backward_h = backward_h.view(-1, backward_h.shape[-1])
    #     backward_input = self.traj_dec_input_backward(G)  # torch.cat([G])
    #     # print('1',backward_input.shape) #torch.Size([128, 20, 256])
    #     # trans = self.transformer_encoder( backward_input)
    #     # backward_input = trans
    #     backward_input = backward_input.view(-1, backward_input.shape[-1])
    #     # print(backward_input.shape) # torch.Size([2560, 256])
    #
    #     for t in range(pred_len - 1, -1, -1):  # 起点是最后一个元素，终点是第一个元素，从后向前遍历
    #
    #         backward_h = self.traj_dec_backward(backward_input, backward_h)
    #         output = self.traj_output(torch.cat([backward_h, forward_outputs[:, t]], dim=-1))
    #         # print(output.shape) #torch.Size([2560, 4])
    ...
    #     backward_outputs = torch.stack(backward_outputs, dim=1)
    #     # for i in range(20):
    #     #     final_emb = self.pos_emb(backward_outputs[:,:,i,:].permute(1, 0, 2)).permute(1, 0, 2)
    #     #     trans = self.transformer_encoder(final_emb)
    #     #     backward_outputs[:, :, i, :] = trans
    #
    #     # print(backward_outputs.shape) #torch.Size([128, 45, 20, 4])
    #
    #     return backward_outputs
    #
